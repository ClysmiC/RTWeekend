...
void iterNext(HashMapIter<K, V> * pIter)
{
	...
	{
		typename hm::Bucket * pBucket = pIter->pHashmap->pBuffer + pIter->iItem;
	...
	K ** ppoKeyInTable=nullptr)		// Only really useful for BiHashMap... don't set this in most cases!
{
	...
	typename hm::Bucket * pBucketIdeal = pHashmap->pBuffer + i;

	// j is the pre-modulo index of the empty bucket slot we are tracking

	uint32_t j = i;
	typename hm::Bucket * pBucketEmpty;

	// Linear probe to the first empty bucket.

	while (true)
	{
		typename hm::Bucket * pBucket = pHashmap->pBuffer + (j % pHashmap->cCapacity);
	...
	while (j - i >= H)
	{
		typename hm::Bucket * pBucketSwappable = nullptr;
		...
		while(k < j)
		{
			typename hm::Bucket * pBucketCandidate = pHashmap->pBuffer + (k % pHashmap->cCapacity);
	...
	K ** ppoKeyFound=nullptr)		// Only really useful for BiHashMap... don't set this in most cases!
{
	...
	{
		typename hm::Bucket * pBucketCandidate = pHashmap->pBuffer + (iCandidate % pHashmap->cCapacity);
	...
	int newCapacity)
{
	...
	typename hm::Bucket * pBufferOld = pHashmap->pBuffer;
	...
	pHashmap->pBuffer = (typename hm::Bucket *)malloc(cBytesNewBuffer);
	...
	if (pBufferOld)
	{
		...
		{
			typename hm::Bucket * pBucket = pBufferOld + i;
...
void iterNext(BiHashMapIter<K, V> * pIter)
{
	...
	{
		typename hm::Bucket* pBucket = pIter->pMapKV->pBuffer + pIter->iItem;
		if (pBucket->infoBits & AlsHash::s_infoOccupiedMask)
		{
			pIter->pKey = &pBucket->key;
			pIter->pValue = pBucket->value;
