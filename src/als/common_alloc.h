...
void init(FixedPoolAllocator<T, Capacity> * pAlloc)
{
    ...
    pAlloc->pFree = reinterpret_cast<typename fpa::FreeListNode *>(pAlloc->aPool);

	for (unsigned int i = 0; i < Capacity; i++)
	{
		typename fpa::FreeListNode * pFreeNode = reinterpret_cast<typename fpa::FreeListNode *>(pAlloc->aPool + i);
		typename fpa::FreeListNode * pFreeNodeNext = (i < Capacity - 1) ? reinterpret_cast<typename fpa::FreeListNode *>(pAlloc->aPool + i + 1) : nullptr;
...
void release(FixedPoolAllocator<T, Capacity> * pAlloc, T * pItem)
{
    ...
    typename fpa::FreeListNode * pFreeNewHead = reinterpret_cast<typename fpa::FreeListNode *>(pItem);
...
void destroy(DynamicPoolAllocator<T, CapacityPerBucket> * pAlloc)
{
	...
	typename dpa::Bucket * pBucket = pAlloc->pBuckets;
	while (pBucket)
	{
		typename dpa::Bucket * pBucketNext = pBucket->pNextBucket;
		delete pBucket;
		pBucket = pBucketNext;
	}
}
...
T * allocate(DynamicPoolAllocator<T, CapacityPerBucket> * pAlloc)
{
	...
	if (!pAlloc->pFree)
	{
		typename dpa::Bucket * pBucketNew = new typename dpa::Bucket;
        ...
        {
            typename dpa::Bucket * pBucketPrev = nullptr;
            typename dpa::Bucket * pBucket = pAlloc->pBuckets;
